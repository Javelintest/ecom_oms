...
const sectionNavLinks = document.querySelectorAll(
  ".sidebar-nav .nav-link[data-section]"
);
...
function handleHashNavigation() {
  ...
  if (hash) {
    const link = document.querySelector(
      `.sidebar-nav .nav-link[data-section="${hash}"]`
    );
    if (link) {
      link.click();
    }
...
async function loadPlatformsList() {
  try {
    ...
    const filters = [
      ...
      "orders-platform-filter",
      "inventory-platform-filter",
    ];
...
async function loadDashboard() {
  try {
    const platformFilter =
      document.getElementById("dashboard-platform-filter")?.value || "";
    ...
    renderPlatformPerformanceTable(
...
function renderKPICards(data) {
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      value:
        "₹ " +
        (data.avg_order_value || 0).toLocaleString("en-IN", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
  ...
  kpiCards.forEach((card) => {
    ...
    cardElement.innerHTML = `
      ...
      ${
        card.change
          ? `<div class="change"><i class="bi bi-arrow-up"></i> ${card.change}</div>`
          : ""
      }
    `;
...
function renderPlatformPerformanceTable(platformData, avgOrderValue) {
  ...
  if (platformData.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="4" class="text-center text-muted py-4"><i class="bi bi-inbox me-2"></i>No platform data available</td></tr>';
    return;
  }
  ...
  platformData.forEach((p) => {
    ...
    tr.innerHTML = `
      ...
      <td class="text-end fw-semibold">₹ ${(p.revenue || 0).toLocaleString(
        "en-IN",
        { minimumFractionDigits: 2, maximumFractionDigits: 2 }
      )}</td>
      <td class="text-end fw-semibold">₹ ${avgOrder.toLocaleString("en-IN", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      })}</td>
    `;
...
function renderRecentOrders(orders) {
  ...
  if (orders.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="text-center text-muted py-4"><i class="bi bi-inbox me-2"></i>No recent orders</td></tr>';
    return;
  }
  ...
  orders.forEach((order) => {
    ...
    tr.innerHTML = `
      ...
      <td><span class="badge bg-${getStatusColor(order.status)} px-3 py-1">${
      order.status
    }</span></td>
      <td class="text-end fw-semibold">${order.currency || "₹"} ${parseFloat(
      order.total || 0
...
async function loadOrders() {
  try {
    const platformFilter =
      document.getElementById("orders-platform-filter")?.value || "";
    ...
    res.data.forEach((o) => {
      const tr = document.createElement("tr");
      const purchaseDate = o.purchase_date
      ...
      tr.innerHTML = `
        ...
        <td>${purchaseDate}</td>
        <td>${o.currency || "₹"} ${parseFloat(o.order_total || 0).toFixed(
        2
      )}</td>
...
async function loadInventory() {
  try {
    const platformFilter =
      document.getElementById("inventory-platform-filter")?.value || "";
    const url = platformFilter
  ...
  try {
    ...
    res.data.forEach((p) => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <button class="btn btn-sm btn-outline-primary" onclick="togglePlatform(${
            p.id
          })">
...
function setupDateFilters() {
  ...
  if (startDateInput) {
    startDateInput.addEventListener("change", () => {
      const customBtn = document.querySelector(
        '.btn-date-filter[data-range="custom"]'
      );
  ...
  const defaultBtn = document.querySelector(
    '.btn-date-filter[data-range="10days"]'
  );
  if (defaultBtn) {
    defaultBtn.click();
  }
...
document
  .getElementById("dashboard-platform-filter")
  ?.addEventListener("change", loadDashboard);
document
  .getElementById("orders-platform-filter")
  ?.addEventListener("change", loadOrders);
document
  .getElementById("inventory-platform-filter")
...
document
  .getElementById("btn-sync-orders")
  ?.addEventListener("click", async () => {
    try {
      ...
      alert(
        "Failed to sync orders: " + (err.response?.data?.detail || err.message)
      );
      const btn = document.getElementById("btn-sync-orders");
      btn.disabled = false;
      btn.innerHTML = '<i class="bi bi-arrow-repeat me-1"></i>Sync All Orders';
...
document
  .getElementById("btn-sync-inventory")
  ?.addEventListener("click", async () => {
    try {
      ...
      const message = `Inventory synced successfully!\nTotal upserted: ${
        res.data.total_upserted
      }\nPlatforms: ${JSON.stringify(res.data.platforms, null, 2)}`;
      alert(message);

      btn.disabled = false;
      btn.innerHTML =
        '<i class="bi bi-arrow-repeat me-1"></i>Sync All Inventory';
    } catch (err) {
