...
function setupEventListeners() {
  ...
  // Request report form
  document.getElementById('request-report-form')?.addEventListener('submit', handleRequestReport);
  
  // Category filter
  document.getElementById('category-filter')?.addEventListener('change', filterReportTypes);
}
  ...
  try {
    ...
    if (selector) {
      ...
      if (platforms.length === 0) {
        selector.innerHTML = '<div class="alert alert-warning mb-0">No platforms configured. Please connect platforms first.</div>';
        return;
      }
    ...
    showAlert('danger', 'Failed to load platforms: ' + (err.response?.data?.detail || err.message));
  }
}
...
async function loadReportTypes(platformId) {
  try {
    ...
    // Update count
    document.getElementById('report-types-count').textContent = availableReportTypes.length;
    
    // Get unique categories
    const categories = [...new Set(availableReportTypes.map(r => r.category))];
    ...
    showAlert('danger', 'Failed to load report types: ' + (err.response?.data?.detail || err.message));
  }
}
...
async function handleRequestReport(e) {
  ...
  try {
    ...
    const res = await axios.post(`${API_BASE_URL}/reports/request`, null, { params });
    
    showAlert('success', `Report requested successfully! Report ID: ${res.data.report_id}`);
    ...
    else if (errorMsg.includes('Unauthorized') || errorMsg.includes('Access to requested resource is denied') ||
             errorMsg.includes('access token') || errorMsg.includes('revoked') || errorMsg.includes('malformed')) {
      showAlert('danger', `
...
async function loadRecentReports(platformId) {
  try {
    ...
    res.data.reports.forEach(report => {
      ...
      const reportTypeInfo = availableReportTypes.find(rt => rt.type === report.report_type);
      const reportName = reportTypeInfo ? reportTypeInfo.name : report.report_type;
      ...
      if (report.processing_status === 'DONE' && report.report_document_id) {
        ...
        `;
      } else if (report.processing_status === 'IN_PROGRESS' || report.processing_status === 'IN_QUEUE') {
        actions = `
      ...
      const createdTime = report.created_time ? new Date(report.created_time).toLocaleString() : 'N/A';
...
async function checkReportStatus(reportId, platformId) {
  try {
    ...
    showAlert('danger', 'Failed to check status: ' + (err.response?.data?.detail || err.message));
  }
}
...
async function downloadReport(reportDocumentId, platformId) {
  try {
    ...
    const res = await axios.get(`${API_BASE_URL}/reports/download/${reportDocumentId}`, {
      params: { platform_id: platformId }
    });
    ...
    showAlert('danger', 'Failed to download report: ' + (err.response?.data?.detail || err.message));
  }
}
