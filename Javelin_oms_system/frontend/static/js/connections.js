...
async function loadConnectionStatus(platformId, platformName) {
  try {
    const res = await axios.get(
      `${API_BASE_URL}/connections/status/${platformId}`
    );
  ...
  statusDiv.innerHTML = `
    ...
    ${
      status.error_message
        ? `<div class="mt-2 small text-danger">${status.error_message}</div>`
        : ""
    }
  `;
...
async function testConnection(platformId, platformName) {
  ...
  try {
    const res = await axios.post(
      `${API_BASE_URL}/connections/test/${platformId}`
    );
    ...
    if (resultDiv) {
      ...
      resultDiv.innerHTML = `
        <i class="bi bi-${
          res.data.status === "success" ? "check-circle" : "x-circle"
        } me-2"></i>
        ${res.data.message}
      `;
    }
...
async function disconnectPlatform(platformId, platformName) {
  if (
    !confirm(
      `Are you sure you want to disconnect ${platformName}? This will remove all stored credentials.`
    )
  ) {
    return;
  }
  ...
  try {
    ...
    showAlert(
      "danger",
      `Failed to disconnect: ${err.response?.data?.detail || err.message}`
    );
  }
}
...
// Amazon OAuth Form
document
  .getElementById("amazon-oauth-form")
  ?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      showAlert(
        "danger",
        `Failed to initiate OAuth: ${err.response?.data?.detail || err.message}`
      );
      e.target.querySelector('button[type="submit"]').disabled = false;
      e.target.querySelector('button[type="submit"]').innerHTML =
        '<i class="bi bi-shield-check me-2"></i>Start OAuth Flow';
    }
  });

// Amazon Manual Connection
document
  .getElementById("amazon-manual-form")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      await axios.post(`${API_BASE_URL}/connections/amazon/manual`, {
        ...
        client_secret: document.getElementById("amazon-manual-client-secret")
          .value,
        refresh_token: document.getElementById("amazon-refresh-token").value,
        marketplace_id: document.getElementById("amazon-manual-marketplace-id")
          .value,
      ...
      showAlert(
        "danger",
        `Connection failed: ${err.response?.data?.detail || err.message}`
      );
      e.target.querySelector('button[type="submit"]').disabled = false;
      e.target.querySelector('button[type="submit"]').innerHTML =
        '<i class="bi bi-check-circle me-2"></i>Connect Amazon';
    }
  });

// Flipkart Connection
document
  .getElementById("flipkart-connection-form")
  ?.addEventListener("submit", async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      await axios.post(`${API_BASE_URL}/connections/flipkart/manual`, {
        ...
        api_secret:
          document.getElementById("flipkart-api-secret").value || null,
      });
      ...
      e.target.querySelector('button[type="submit"]').disabled = false;
      e.target.querySelector('button[type="submit"]').innerHTML =
        '<i class="bi bi-check-circle me-2"></i>Connect Flipkart';
    }
  });
...
document
  ...
  ?.addEventListener("click", async () => {
    ...
    btn.disabled = true;
    btn.innerHTML =
      '<i class="bi bi-arrow-clockwise spin me-2"></i>Refreshing...';
...
function fillSandboxToken() {
  ...
  showAlert(
    "info",
    "Sandbox token and region have been filled. Please enter your Client ID and Client Secret to complete the connection."
  );
}
